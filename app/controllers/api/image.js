...
const index = async (ctx, _next) => {
  let { page, perPage, category, type } = ctx.request.query;
...
const create = async (ctx, _next) => {
  ...
  let { title, uniqueKey, author, preface, desc, content, cover, type, tag, isBanned, isPrivate, isAdminOnly, articleCategory, sequence } = ctx.state.articleParams;
  if (content == '') {
    content = ' ';
  }
  let data = {
...
const update = async (ctx, _next) => {
  ...
  let mongoose = require('mongoose');
  let userId = mongoose.Types.ObjectId(currentUser.id);
  let { id, title, uniqueKey, author, preface, desc, content, cover, type, tag, isBanned, isPrivate, isAdminOnly, articleCategory, sequence } = ctx.state.articleParams;
  if (content == '') {
    content = ' ';
  }
  let data = {
...
const checkParamsBody = async (ctx, next) => {
  ...
  console.log(2222222);
  let { id, title, uniqueKey, author, preface, desc, content, cover, type, tag, isBanned, isPrivate, isAdminOnly, articleCategory, sequence } = body;
  if (title == '') {
    return false;
  }
